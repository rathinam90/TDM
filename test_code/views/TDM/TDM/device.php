  ...
  var device_type=document.getElementById("device_type").value;
    ...
    var getres=result["responseData"];
      ...
      $.each(getres, function(i, item) {
        ...
        // document.getElementById("tbl_devices").innerHTML += "<tr><td>"+(getres[i].id)+"</td><td><form id=myform"+(getres[i].device_id)+" action=device method=POST><input type=hidden value="+(getres[i].device_id)+" id=hid name=hid></input><a id=dev_id name=dev_id href=# onclick=document.getElementById('myform"+(getres[i].device_id)+"').submit();>"+(getres[i].device_id)+"</a></form> </td><td>"+(getres[i].make)+"</td><td>"+(getres[i].type)+"</td><td>"+(getres[i].os)+"</td><td>"+(getres[i].version)+"</td><td><span class=label label-success>Verified</span></td><td>Jinesh</td><td>"+(date)+"</td></tr>";
      })
